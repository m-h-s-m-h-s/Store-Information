  ...
  async getStoreInformation(request: StoreInfoRequest): Promise<StoreInfoResponse> {
    try {
      ...
      const completion = await this.client.chat.completions.create({
        model: 'gpt-5-mini', // Use mini for chat completions
        messages: [
          {
            role: 'system',
